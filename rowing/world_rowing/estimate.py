...
def tridiagonal_matmul(dl, d, du, b):
    return (
        ...
        + jnp.roll(b * jnp.roll(du.at[..., -1].set(0), 1, axis=-1), -1, axis=-1)
    )
...
def ffill(x, init=None, **kwargs):
    ...
    return lax.scan(something, init, x, **kwargs)[1]
...
def _step_dist(carry, xs):
    ...
    dxi_prec = jnp.where(dxi_update, pi_prec / (pi_prec * dti_var / pi**2 + dti**2 / pi**4 + dti_var / pi**4), dxi_prec)
    xi_update = ~jnp.isfinite(x1)

    xi = jnp.where(xi_update, x0 + dxi, x1)
    xi_prec = jnp.where(xi_update, x0_prec * dxi_prec / (x0_prec + dxi_prec), x1_prec)
...
def estimate_times(
    ...
    id, iboat = jnp.meshgrid(jnp.arange(inter_dists.size), boats, indexing="ij")
    ...
    insertat = jax.vmap(jnp.searchsorted, (1, None))(dist, inter_dists)[ibf, idf]
    insertedat = insertat + jnp.arange(insertat.size)
    points = jnp.ones(len(dist) + insertedat.size, bool).at[insertedat].set(False)

    inter_x = jnp.where(ibf[:, None] == boats[None, :], inter_dists[idf][:, None], jnp.nan)
    inter_x_prec = jnp.where(ibf[:, None] == boats[None, :], 1 / inter_dist_var, 0)

    x0 = jax.vmap(jnp.insert, (1, None, 1))(dist, insertat, inter_x).T
    x0_prec = jax.vmap(jnp.insert, (1, None, 1))(1 / dist_var, insertat, inter_x_prec).T

    p = ffill(jax.vmap(jnp.insert, (1, None, None))(pace, insertat, jnp.nan).T)
    p_var = ffill(jax.vmap(jnp.insert, (1, None, None))(pace_var, insertat, jnp.nan).T)
    ...
    _, (x, x_prec, p, p_prec, dx, dx_var) = jax.lax.scan(_step_dist, carry, (x0, x0_prec, p, p_prec))
    x_var = 1 / x_prec
    p_var = 1 / p_prec
    ...
    dt_var += jnp.nansum(pdx_prec * jnp.square(pdx - dt[:, None]), axis=1) * dt_var
    ...
    t_prec = jnp.full_like(t, t_prior_prec).at[insertedat].set(1 / inter_time_var)
    prec_t = t * t_prec + tridiagonal_matmul(bbdl, bbd, bbdu, est_t)
    prec_d = bbd + t_prec
    post_t = jax.lax.linalg.tridiagonal_solve(bbdl, prec_d, bbdu, prec_t[..., None])[..., 0]
    post_t_covar = jax.lax.linalg.tridiagonal_solve(
        bbdl, prec_d, bbdu, jax.lax.linalg.tridiagonal_solve(bbdl, prec_d, bbdu, jnp.eye(len(bbd))).T
    )
    post_t = jax.lax.cummax(post_t)

    post_dt = tridiagonal_matmul(bl, bd, b0, post_t)
    post_dt_covar = tridiagonal_matmul(bl, bd, b0, tridiagonal_matmul(bl, bd, b0, post_t_covar).T)
...
def parse_livetracker_data(data):
    ...
    inters = intermediates[["distance", "ResultTime"]].stack().swaplevel()
    inters["ResultTime"] = inters.ResultTime.dt.total_seconds()
    inters["distance"] = inters.distance.astype(int)
    inters = inters.reset_index().set_index(["distance", "boat"]).ResultTime.unstack()[live_pace.columns]
    inters.loc[0] = 0
    inters = inters.sort_index()[live_pace.columns]

    speed = live_speed.ffill(axis=0).values
    speed_var = jnp.where(live_pace.notna().values, 1e-2 / 12, 100).at[0].set(10)

    pace = 1 / speed
    pace_var = speed_var / speed**4

    dist = live_distance.values
    dist_var = jnp.where(live_distance.notna().values & (dist < race_distance), 1 / 12, 1e2).at[dist == race_distance].set(1)
...
def parse_livetracker_times(data, inter_time_var=1e-4 / 12, inter_dist_var=1e-4 / 12, t_prior_prec=0):
    live_boat_data = livetracker.parse_livetracker_data(data)
    intermediates = livetracker.parse_intermediates_data(data)
    race_distance = data["config"]["plot"]["totalLength"]
    ...
    speed_var = jnp.where(jnp.isfinite(speed_var), speed_var, 100).at[0].set(10)
