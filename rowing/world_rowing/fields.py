...
FIELD_NAMES = {k.casefold(): v for k, v in list(field_names.items())}
...
def format_datatype(data, **formatters):
    ...
    return pd.concat({col: values.apply(formatters.get(datatypes[col], identity)) for col, values in data.items()}, axis=1)


streamlit_formatters = {"timedelta": utils.format_timedelta, "percentage": "{:,.2%}".format}
...
plotly_formatters = {"timedelta": to_timestamp}


def to_plotly_dataframe(data):
    plotly_data = format_datatype(data, **plotly_formatters)
    plotly_data = plotly_data.map(lambda x: plotly_formatters.get(infer_dtype([x]), identity)(x))
    return plotly_data
...
def filter_numerical_columns(data):
    dtypes = data.dtypes
    return dtypes.index[dtypes.map(is_numeric_dtype) | dtypes.map(is_datetime64_any_dtype) | dtypes.map(is_timedelta64_dtype)]
