...
def main(params=None):
    ...
    with st.expander("Select Results"):
        select_competitions, set_gmts, filter_results = st.tabs(["Select competition", "Set GMTs", "Filter Results"])
    ...
    results[fields.raceBoatIntermediates_ResultTime] = results[fields.raceBoatIntermediates_ResultTime] + pd.Timestamp(0)
    ...
    with st.expander("Filter Intermediate Results"):
        ...
        intermediate_results = res.groupby([fields.Race, fields.raceBoats, fields.Distance]).first().unstack(-1)
    ...
    cols = st.multiselect(
        "select which data to show", options=col_formats, default=[fields.PGMT, fields.raceBoatIntermediates_ResultTime]
    )
