...
@cache
def find_country_iso2(country, data_path=_data_path / "iso_country.json"):
    return get_iso2_names(data_path)[country]


@cache
def get_flag_im(country, data_path=_data_path / "iso_country.json", flag_path=_flag_path):
    import matplotlib.pyplot as plt

    iso2cnt = find_country_iso2(country, data_path)
    im = plt.imread(flag_path / f"{iso2cnt}.png")
    return im
...
def make_flag_box(
    ...
    **kwargs,
    ...
    offset_im = OffsetImage(im, zoom=zoom * base_width / im.shape[1], resample=resample, **(offset_kws or {}))
    return AnnotationBbox(offset_im, xy, frameon=frameon, xycoords=xycoords, box_alignment=box_alignment, pad=0, **kwargs)
...
def extract_fields(record, fields):
    return {k: getnesteditem(record, *items) for k, items in fields.items()}
...
def merge(dfs, **kwargs):
    ...
    for right in dfs[1:]:
        kws = {k: next(val) for k, val in iter_kwargs.items()}
