...
def _rsc_phase_names():
    ...
    final_codes.update({f"000{i + 1}": " " + chr(i + 65) for i in range(10)})
    semi_codes = {f"000{2 * i + 1}00": " " + chr(2 * i + 65) + "/" + chr(2 * i + 66) for i in range(13)}
    semi_codes.update((f"000{2 * i + 2}00", " " + chr(2 * i + 65) + "/" + chr(2 * i + 66)) for i in range(13))
    semi_codes.update((f"000{2 * i + 1}", " " + chr(2 * i + 65) + "/" + chr(2 * i + 66)) for i in range(13))
    semi_codes.update((f"000{2 * i + 2}", " " + chr(2 * i + 65) + "/" + chr(2 * i + 66)) for i in range(13))
    semi_codes["005100"] = " A/B rerow"
    rep_codes = default_codes.copy()
    rep_codes.update({f"005{i + 1}00": "" for i in range(9)})
    rnd_codes = {f"{i + 1}000{j + 1}00": "" for i in range(3) for j in range(9)}
    phase_codes = {"FNL-": final_codes, "SFNL": semi_codes, "REP-": rep_codes, "RND": rnd_codes}
    return {p + c: name + code for p, name in phase_name.items() for c, code in phase_codes.get(p, default_codes).items()}
...
def parse_race_codes(race_codes):
    ...
    codes = race_codes.str.extract(r"ROW([MWX])([A-Z]+[0-9])-+([A-Z0-9]*)-+([A-Z]+-?[0-9]+)", expand=True)
    return pd.concat(
        {
...
def prepare_params(**kwargs):
    return {field: value for key, options in kwargs.items() for field, value in prepare_options(key, options)}


if use_requests:
    from rowing.utils import make_http_session

    # Shared session: per-request timeout plus retry/backoff on transient errors.
    _session = make_http_session()

    def load_json_url(url, params=None, timeout=20.0, **kwargs):
...
def load_competition_best_times(json_url, **kwargs):
    return pd.DataFrame.from_records(
        extract_fields(record, WBT_RECORDS) for record in load_json_url(json_url, **kwargs)["BestTimes"]
...
def get_worldrowing_data(*endpoints, cached=True, request_kws=None, **kwargs):
    if cached:
        hashable_kws = {
            k: vals if isinstance(vals, (str, int, float, tuple)) else tuple(dict(vals).items()) for k, vals in kwargs.items()
        }
...
def get_worldrowing_record(*endpoints, **kwargs):
    ...
    records = parse_records(endpoints[:-1], [data])
    return records.loc[0]
...
def parse_records(endpoints, records):
    ...
    records = pd.concat([pd.json_normalize(r) for r in records]).reset_index(drop=True)
...
def parse_races(races):
    ...
    if fields.race_event_boatClassId in races.columns:
        boat_classes = races[fields.race_event_boatClassId].replace(BOATCLASSES).rename(fields.race_boatClass)
...
INCLUDE_RACE = "event.competition,raceStatus,racePhase,raceBoats.raceBoatIntermediates.distance,event"
INCLUDE_RACE_ATHLETES = INCLUDE_RACE + ",raceBoats.raceBoatAthletes,raceBoats.raceBoatAthletes.person"
...
def get_race(race_id, athletes=False, **kwargs):
    kwargs.setdefault("sort", (("eventId", "asc"), ("Date", "asc")))
    kwargs.setdefault(
        "include",
        INCLUDE_RACE_ATHLETES if athletes else INCLUDE_RACE,
    )
    data = get_worldrowing_data("race", race_id, **kwargs)
...
def extract_results(races):
    race_results = (
        pd.concat(races[fields.race_raceBoats].map(pd.json_normalize).values)
        .reset_index(drop=True)
        .rename(columns=fields.renamer("raceBoats"))
    )

    race_intermediates = pd.json_normalize(race_results[fields.raceBoats_raceBoatIntermediates].sum()).rename(
        columns=fields.renamer("raceBoatIntermediates")
    )

    if not race_intermediates.empty:
        race_intermediates = race_intermediates.join(
            race_results.set_index(fields.raceBoats_id), on=fields.raceBoatIntermediates_raceBoatId
        )
        race_intermediates[fields.raceBoats_ResultTime] = read_times(race_intermediates[fields.raceBoats_ResultTime])
        race_intermediates[fields.raceBoatIntermediates_ResultTime] = read_times(
            race_intermediates[fields.raceBoatIntermediates_ResultTime]
        )

        inter_distances = race_intermediates[fields.raceBoatIntermediates_distance].str.extract("([0-9]+)")[0].astype(float)
        ...
        race_distances = (
            ...
            .groupby(fields.raceBoats_raceId)[fields.Distance]
            .max()
        )

        race_intermediates[fields.race_distance] = race_distances.reindex(race_intermediates[fields.raceBoats_raceId]).values

        race_intermediates = race_intermediates.dropna(subset=[fields.Distance])
        race_intermediates[fields.Distance] = race_intermediates[fields.Distance].astype(int)
...
def get_competitions(year=None, fisa=True, has_results=True, flat_water=True, cached=True, **kwargs):
    ...
    kwargs["include"] = ",".join(set(kwargs.get("include", "CompetitionType").split(",") + "competitionType,venue".split(",")))

    competitions = get_worldrowing_records("competition", cached=cached, **kwargs)
    if competitions.empty:
        return competitions

    competitions[fields.WBTCompetitionType] = competitions[fields.competition_competitionType].replace(COMPETITION_TYPES)
    if flat_water:
        is_flat_water = competitions[fields.competition_competitionType].isin(COMPETITION_TYPES)
...
def get_live_competitions(fisa=True, has_results=True, filter=None, filterOptions=None, cached=True, **kwargs):
    ...
    return get_competitions(
        filter=filter, filterOptions=filterOptions, fisa=fisa, has_results=has_results, cached=cached, **kwargs
    )
...
def get_most_recent_competition(fisa=True, has_results=True):
    ...
    logger.info(f"loaded most recent competition: {competition.competition}")
    return competition
...
def get_live_race(fisa=True, competition=None):
    ...
    for _, race in live_races.iterrows():
        try:
            data = get_worldrowing_data("livetracker", "live", race.race_id, cached=False)
            lanes = data["config"].get("lanes", [])
            started = data.get("live") or any(lane.get("live") for lane in lanes)
...
def get_last_races(n=1, fisa=True, competition=None, cached=False):
    ...
    if competition is None:
        ...
        if len(competitions):
            races = pd.concat(
                [get_races(competition.competition_id, cached=cached) for _, competition in competitions.iterrows()]
            )
        else:
            competition = get_most_recent_competition(fisa)
    ...
    if not races.empty:
        ...
        return races.drop_duplicates(subset=["Race", "race_id"]).loc[started].sort_values("race_DateString").iloc[-n:]


def get_next_races(n=1, fisa=True, competition=None):
    competitions = get_live_competitions(fisa=fisa, has_results=True)
    if len(competitions):
        races = pd.concat([get_races(competition.competition_id) for _, competition in competitions.iterrows()])
...
def show_next_races(n=10, fisa=True, competition=None):
    next_races = get_next_races(n, fisa=fisa, competition=competition)[["DisplayName", "DateString"]].reset_index(drop=True)
    next_races.DateString = next_races.DateString.dt.tz_convert(CURRENT_TIMEZONE)
    next_races.columns = ["Race", "Time"]
    now = datetime.datetime.now().astimezone(CURRENT_TIMEZONE)
    next_races["Time to race"] = (next_races.Time - now).apply(format_timedelta_hours)
    return next_races
...
def get_race_results(race_id=None, event_id=None, competition_id=None, cached=True, **kwargs):
...
def get_intermediate_results(race_id=None, event_id=None, competition_id=None, cached=True, **kwargs):
    filters = tuple(dict(kwargs.pop("filter", ())).items())
    if race_id:
        filters += (("id", race_id),)
    if event_id:
    ...
    results = pd.DataFrame.from_records(
        [
            ...
            for boat in race["raceBoats"]
            if boat["ResultTime"]
            for inter in sorted(boat["raceBoatIntermediates"], key=lambda x: x["ResultTime"])
        ]
    )
...
def get_race_intermediates(race_id, race_distance="d2000m"):
    ...
    full_intermediates["Distance"] = full_intermediates["distance"].str.extract(r"(\d+)")[0].astype(int)

    full_inters = full_intermediates.groupby(["Distance", "Country"]).first().unstack()
    return full_inters
...
def get_race_athletes(race_id):
    ...
    if "raceBoatAthletes" in race_boats:
        race_boat_athletes = pd.json_normalize(race_boats.raceBoatAthletes.sum()).join(
            race_boats.set_index("id").DisplayName.rename("Boat"), on="raceBoatId"
        )
        return race_boat_athletes.set_index(["Boat", "boatPosition"])["person.DisplayName"].unstack()
...
@cache
def get_competition_best_times(timeout=2.0, load_event_info=False):
    wbt_stats = get_worldrowing_records("statistic", cached=True, filter=(("Category", "WBT"),))
    ...
    wbts = pd.concat(wbts, names=[fields.bestTimes_CompetitionType]).reset_index(0).reset_index(drop=True)
    wbts[fields.WBTCompetitionType] = wbts[fields.bestTimes_CompetitionType].replace(WBT_TYPES)
    ...
    if load_event_info:
        events, errors = map_concurrent(
            lambda event_id: get_worldrowing_record("event", event_id, include="competition.competitionType"),
            {i: (i,) for i in wbts[fields.bestTimes_EventId]},
        )
        if errors:
            logger.warning("get_competition_best_times.load_event_info errors=%s", errors)
...
def get_world_best_times():
    return get_competition_best_times().sort_values(fields.bestTimes_ResultTime).groupby(fields.bestTimes_BoatClass).min()
...
def find_world_best_time(
    ...
    if boat_class is None:
        if event is None:
            if race is None:
                if race_id is None:
                    raise ValueError("must pass at least one of boat_class, event, race or race_id")
                race = get_worldrowing_data("race", race_id)
            event = get_worldrowing_data("event", race["eventId"])
        boat_class = get_worldrowing_data("boatClass", event["boatClassId"])["DisplayName"]
...
def merge_competition_results(
    ...
    event_wbts = pd.merge(
        competition_events,
        boat_wbts,
        left_on=fields.event_boatClassId,
    ...
    if fields.raceBoatIntermediates_Rank in race_results:
        race_data = pd.merge(
            ...
            race_wbts,
            left_on=race_id,
            right_on=fields.race_id,
            suffixes=("", "_1"),
        )
    elif "ResultTime" in race_results:
        race_data = pd.merge(
            race_results.loc[race_results[fields.Rank].notna() & (race_results["ResultTime"] > pd.Timedelta(0))],
            race_wbts,
            left_on=race_id,
            right_on=fields.race_id,
            suffixes=("", "_1"),
        ...
        race_data = pd.merge(
            race_results.loc[race_results[fields.Rank].notna()],
            race_wbts,
            left_on=race_id,
            right_on=fields.race_id,
            suffixes=("", "_1"),
    ...
    if fields.race_distance in race_data:
        race_data[fields.race_distance] = race_data[fields.race_distance].where(
            race_data[fields.race_distance].notna(), race_data.groupby("event_id")[fields.race_distance].transform("max")
        )
        race_data[fields.race_distance] = race_data[fields.race_distance].where(
            race_data[fields.race_distance].notna(), race_data[fields.race_distance].max()
        )
    if fields.raceBoatIntermediates_Rank in race_data:
        race_data[fields.raceBoatIntermediates_Rank] = race_data[fields.raceBoatIntermediates_Rank].astype(int)
    if fields.raceBoatIntermediates_distance in race_data.columns:
        race_data[fields.PGMT] = (
            ...
            * race_data[fields.Distance]
            / race_data[fields.race_distance]
        )
    else:
        time_col = "ResultTime"
        if fields.ResultTime in race_data:
            time_col = fields.ResultTime
        race_data[fields.PGMT] = race_data[fields.GMT].dt.total_seconds() / race_data[time_col].dt.total_seconds()
...
def get_competition_results(competition_id=None, with_intermediates=True):
    race_data = get_raw_competition_results(competition_id=competition_id, with_intermediates=with_intermediates)
    if race_data.empty:
        return race_data
    if with_intermediates:
        race_summaries = race_data.set_index(["Boat Class", "Event", "Race", "Phase", "distance", "Rank"])[
            [
                "Country",
                "Lane",
                "ResultTime",
        ...
        race_summaries = race_data.set_index(["Boat Class", "Event", "Race", "Phase", "Rank"])[
            [
                "Country",
                "Lane",
                "ResultTime",
...
def get_competition_pgmts(competition_id=None, finals_only=False):
    ...
    if finals_only:
        competition_races = competition_races.loc[competition_races.racePhaseId == RACE_PHASES["Final"]]
    ...
    competition_results["PGMT"] = competition_results.worldBestTime / competition_results.ResultTime
    ...
    results.columns = ["Boat", "PGMT", "Time", "WBT", "Country", "Rank", "Lane", "Date"]
