...
from . import files, geodesy, splits, utils
...
class LudumClient(utils.CachedClient):
    ...
    def __init__(
        ...
        super().__init__(username=username, password=password, path=path, local_cache=local_cache, map_kws=map_kws)
    ...
    def authenticate(self, username, password, client_secret, club_id):
        ...
        s = self.session.post(
            self.https_base_url + "/api/v2/login", data=payload, headers=self._default_headers, timeout=self.timeout
        )
        s.raise_for_status()
    ...
    def post(self, endpt, data=None, json=None, **kwargs):
        ...
        self.n_requests += 1
        return self.session.post(self.https_base_url + endpt, data=data, json=json, timeout=self.timeout, **kwargs)
    ...
    def get(self, endpt, **kwargs):
        kwargs["headers"] = self.prepare_headers(**kwargs.get("headers", {}))
        self.n_requests += 1
        return self.session.get(self.https_base_url + endpt, timeout=self.timeout, **kwargs)
    ...
    def cached_get(self, key, endpt, local_cache=None, path=None, reload=False, **kwargs):
        ...
        if local_cache:
            if reload:
                return local_cache.update(key, path, self.get_json, endpt, **kwargs)
            else:
                return local_cache.get(key, path, self.get_json, endpt, **kwargs)
    ...
    def cached_post(self, key, endpt, local_cache=None, path=None, reload=False, **kwargs):
        ...
        if local_cache:
            if reload:
                return local_cache.update(key, path, self.post_json, endpt, **kwargs)
            else:
                return local_cache.get(key, path, self.post_json, endpt, **kwargs)
    ...
    def cached_json(self, key, url, local_cache=None, path=None, reload=False, **kwargs):
        ...
        if self.local_cache:
            if reload:
                return local_cache.update(key, path, self.download_json, url, **kwargs)
            else:
                return self.local_cache.get(key, path, self.download_json, url, **kwargs)

        return self.download_json(url, **kwargs)

    def get_session_data(self, session_id, reload=True, **kwargs):
        fields = ",".join(session_fields)
        endpt = f"/api/v2/session/{session_id}?fields=id|name|seat_racing|session&include={fields}"
        return self.cached_get(session_id, endpt, **kwargs, path=self.path / "session", reload=True)
    ...
    def get_agenda(self, start=None, end=None, period="60d", **kwargs):
        ...
        logger.info("getting agenda from %s to %s", start.strftime("%Y-%m-%d"), end.strftime("%Y-%m-%d"))
        agenda, errors = utils.map_concurrent(self.get_date_agenda, dict(zip(dates, zip(dates))), **self.concurrent_kws)
    ...
    def load_agenda(self, start=None, end=None, days=60, **kwargs):
        data = self.get_agenda(start=start, end=end, days=days, **kwargs)
        agenda_data = sum((d["data"]["data"] for d in data.values()), [])
        return pd.json_normalize(agenda_data)

    def load_sessions(self, start=None, end=None, days=60, reload=True, **kwargs):
        agenda = self.load_agenda(start=start, end=end, days=days, **kwargs)
        logger.info("loading %d sessions", len(agenda))
        sessions, errors = self.map_concurrent(
            self.get_session_data, dict(zip(agenda.id, zip(agenda.id))), reload=True, **self.concurrent_kws
        )
        errors and logger.warning("load_sessions experienced %r", errors)
        return agenda, sessions

    def load_session_activity_files(
        self, session_id, user_id, gps_file=None, gps_file_url=None, hr_file=None, hr_file_url=None, **kwargs
    ):
        urls = {
            "gps_data": gps_file,
        ...
        loaded = ((k, self.cached_json(("activity", session_id, user_id, k), url)) for k, url in urls.items() if url)
        return {k: activity for k, activity in loaded if activity}

    def load_session_activities_files(self, sessions):
        arg_names = ("session_id", "user_id", "gps_file", "gps_file_url", "hr_file", "hr_file_url")
        inputs = ((k, [data.get(c) for c in arg_names]) for k, data in sessions.items())
        ...
        activity_files, errors = self.map_concurrent(
            self.load_session_activity_files,
            inputs,
            **self.concurrent_kws,
        )
    ...
    def load_session_gps_data(self, session_id, user_id, gps_file):
        gps_data = self.load_session_activity_files(session_id, user_id, gps_file)["gps_data"]
        return parse_ludum_gps_data(gps_data)

    def load_sessions_gps_data(self, sessions):
        arg_names = ("session_id", "user_id", "gps_file")
        inputs = ((k, [data.get(c) for c in arg_names]) for k, data in sessions.items())
        ...
        activities, errors = self.map_concurrent(
            self.load_session_gps_data,
            inputs,
            **self.concurrent_kws,
        )
        errors and logger.warning("load_sessions_gps_data experienced %r", errors)
        metadata = pd.concat({k: data[0] for k, data in activities.items()}, axis=0).droplevel(1, axis=0)
        positions = pd.concat({k: data[1] for k, data in activities.items()}, axis=0).dropna(subset=["latitude", "longitude"])
        return metadata, positions

    def load_morning_monitoring(self, dates, ids=(), squad_ids=(), **kwargs):
        load_date = partial(self.get_morning_monitoring, ids=ids, squad_ids=squad_ids)
        mm_data, errors = utils.map_concurrent(
            load_date, dict(zip(dates, zip(dates))), path=self.path / "morning_monitoring", **self.concurrent_kws, **kwargs
        )
    ...
    def get_morning_monitoring(self, date, ids=(), squad_ids=(), path=None):
        ...
        data = pd.json_normalize(
            [{**{k: d[k] for k in ["athlete_id", "username", "athlete_name"]}, **d["daily"][date]} for d in json_data["data"]]
        )
        data["date"] = date
        return data
    ...
    def get_squads(self, **kwargs):
        params = {
            "club_id": self.club_id,
            "include": ("squad_users.user:fields(id%7Cfirst_name%7Clast_name%7Cfull_name),squad_users.user.current_club_user"),
            "limit": "1000",
        }
        ...
        for squad_data in data["data"]["data"]:
            ...
            squad_data["squad_users"] = pd.json_normalize(squad_data["squad_users"]["data"])
    ...
    def get_ergo_data(self, start_date=None, end_date=None, period="365d", distances=(2000, 5000), times=(), **kwargs):
        ...
        raw_data = self.cached_post("ergo_data", "/api/v2/ergo-report", json=params, headers=headers)
        ergo_data = pd.concat(
            {
                athlete["user_name"]: pd.json_normalize(sum((event["pieces"] for event in athlete["events"].values()), []))
...
def extract_session_data(sessions):
    ...
    session_files = pd.concat(
        {
            session: pd.DataFrame.from_records(
                [
                    {**{"full_name": d["full_name"]}, **dict(utils.flatten_json(d["session_individual_files"]["data"]))}
                    for d in data.get("data", {}).get("users", {}).get("data", [])
                    if d
                ]
            )
    ...
    for c in session_files.columns[session_files.columns.str.endswith("id") & ~session_files.columns.str.endswith("guid")]:
...
def extract_session_info(sessions):
    return pd.concat([pd.json_normalize(data["data"]) for i, data in sessions.items()], ignore_index=True).set_index("id")
...
def extract_crew_member(session_info):
    crew_info = (
        pd.concat(
            dict(session_info.loc[session_info["crews.data"].apply(len) > 0, "crews.data"].apply(pd.json_normalize).items()),
            names=["session_id", ""],
        )
        .droplevel(1)
        .reset_index()
    ...
    crew_info["boat_class"] = (
        crew_info["weight.data.name"]
        .replace({"Heavyweight": "", "Lightweight": "L"})
        .str.cat([crew_info["gender.data.short"], crew_info["boat_type.data.name"]])
    )
    crew_info["gmt"] = pd.to_timedelta(crew_info["gmt_score.data.gmt_score"], unit="s")

    member_info = pd.concat(
        dict(crew_info.loc[crew_info["crew_users.data"].apply(len) > 0, "crew_users.data"].apply(pd.json_normalize).items()),
        ignore_index=True,
    ).set_index("id")
...
def extract_rowers(sessions):
    return pd.concat(
        {
            (rower["user_id"], session_id): pd.json_normalize({**crew, **rower})
...
def merge_activity_info(agenda, session_info, session_files, crew_info, member_info):
    activity_info = session_files.join(
        member_info.join(
            crew_info[["session_id", "name", "boat_class", "gmt", "parent_id"]].rename(columns={"name": "boat_name"}),
            on="crew_id",
        ).set_index(["session_id", "user_id"])[["boat_name", "boat_class", "gmt"]],
        on=["activity_id", "user_id"],
    ).join(agenda.set_index("id")[["start_date", "name"]], on="activity_id")
...
def merge_index(index, activity_info):
    ...
    merged_index = index_frame.join(
        activity_info[["id", "name", "start_date", "full_name", "boat_name", "boat_class", "gmt"]].set_index("id"),
        on="activity_id",
    ).rename(
        columns={
            "name": "session",
...
def find_best_times(positions, activity_info, cols=None, max_distance=20, max_group=4, max_order=20):
    ...
    logger.info("finding best times for %d sessions with %d points", session_positions.ngroups, len(positions))
    ...
    best_times = pd.concat(session_best_times, names=["activity_id", "length", "distance"]).reset_index()
    best_times["order"] = best_times.groupby(["activity_id", "length"]).cumcount() + 1
    best_times = best_times.set_index(["activity_id", "length", "order"]).unstack(level=[1, 2]).T.unstack(level=0)

    gmts = activity_info.loc[activity_info.id.isin(best_times.columns.levels[0])].set_index("id").gmt.dropna()
    best_splits = best_times.xs("split", level=1, axis=1)[gmts.index].apply(
        lambda s: s.fillna(pd.Timedelta(0)).dt.total_seconds()
    )
    pgmts = (gmts.dt.total_seconds().values[None] / best_splits / 4).replace(np.inf, 0)

    best_times = pd.concat([best_times, pd.concat({"pgmt": pgmts}, axis=1).swaplevel(axis=1)], axis=1)

    best_times.columns.names = "activity_id", "data"

    best_times.columns = (
        best_times.columns.to_frame()
        ...
        .set_index(["start_date", "session", "boat_class", "gmt", "boat", "athlete", "data"])
        .index
    )

    distance_orders = (
        (s, d, i * max_group + j)
        for i in range(max_order // max_group)
        for s, d in splits._STANDARD_DISTANCES.items()
        for j in range(max_group)
    )
    distance_orders = pd.MultiIndex.from_tuples(
        [(s, n + 1) for s, d, n in distance_orders if n * d <= max_distance], names=["interval", "rank"]
    )
    return best_times.sort_index(axis=1, ascending=[False] + [True] * (best_times.columns.nlevels - 1)).reindex(
        distance_orders
    )
...
def find_crossings(positions, activity_info, loc=None):
    ...
    session_positions = positions.groupby(level=0)
    logger.info("finding crossings for %d sessions with %d points", session_positions.ngroups, len(positions))
    ...
    crossing_data = pd.concat(
        session_crossing_data, names=["activity_id", "leg", "location", "landmark", "distance"]
    ).reset_index("distance")
    crossing_data.columns.name = "data"

    crossing_data["leg_time"] = crossing_data.time - crossing_data.groupby(level=[0, 1]).time.min().reindex(
        crossing_data.index
    )

    zero_index = crossing_data.leg_time == pd.Timedelta(0)

    crossing_data["timeDelta"] = crossing_data.time - crossing_data.time.shift()
    ...
    crossing_data["split"] = pd.to_timedelta(
        crossing_data.timeDelta.dt.total_seconds() / crossing_data.distanceDelta / 2, unit="s"
    )
    crossing_data.loc[zero_index, "split"] = pd.Timedelta(0)

    crossings = (
        crossing_data[["time", "leg_time", "distanceDelta", "timeDelta", "split", "cadence", "bearing", "heart_rate"]]
        .stack("data")
        .unstack(locations.index.names)
        .reindex(locations.index, axis=1)
        .T
    )
    crossings.columns = merge_index(crossings.columns, activity_info)
    return crossings.swaplevel(-2, -1, axis=1).sort_index(axis=1, ascending=[False] + [True] * (crossings.columns.nlevels - 1))


def load_morning_monitoring(api: LudumClient, start, end=None, squad_name=None, squad_id=None, **kwargs):
    if squad_name:
    ...
    dates = [date.strftime("%Y-%m-%d") for date in pd.date_range(start=start, end=end)]
    ...
    return mm_data.loc[mm_data.athlete_entered_monitoring, mm_cols].set_index(["athlete_name", "date"]).sort_index()


def get_parser():
    parser = argparse.ArgumentParser(description="Analyse recent ludum data")
    ...
    parser.add_argument("--excel-file", type=Path, default="ludum.xlsx", nargs="?", help="path of output excel spreadsheet")
    utils.add_gspread_arguments(parser)
    parser.add_argument("--folder", type=str, default=None, nargs="?", help="folder path to download fit files")
    parser.add_argument(
        "--start-date", type=pd.to_datetime, help="start date to search for activities from in YYYY-MM-DD format"
    )
    parser.add_argument(
        "--end-date",
        type=pd.to_datetime,
    ...
    parser.add_argument("--period", type=pd.to_timedelta, default="1w", help="Period to search back from")
    parser.add_argument("--login", type=bool, default=True, help="Whether to log in to Ludum or just use the local cache")
...
def run(args):
    ...
    if options.credentials:
        api = LudumClient.from_credentials(options.credentials, local_cache=local_cache, path=options.folder)
    else:
        api = LudumClient(username=options.user, password=options.password, local_cache=local_cache, path=options.folder)
    ...
    if calc_best_times or calc_crossings or reload:
        agenda, sessions = api.load_sessions(options.start_date, options.end_date, reload=options.login)
    ...
    if calc_best_times:
        try:
            best_times = find_best_times(positions, activity_info, cols=["cadence", "bearing", "heart_rate"])
    ...
    if any([morning_monitoring, calc_best_times, calc_crossings]):
        ...
        if options.gspread:
            ...
            if isinstance(best_times, pd.DataFrame):
                ...
                gsheet_best_times.loc[:, (slice(None),) * 6 + ("pgmt",)] = (
                    gsheet_best_times.loc[:, (slice(None),) * 6 + ("pgmt",)]
                    .fillna(0)
                    .applymap("{:.2%}".format)
                    .replace("0.00%", "")
            ...
            if isinstance(crossings, pd.DataFrame):
                gsheet_crossings = utils.format_gsheet(crossings.swaplevel(-2, -1, axis=1).sort_index(axis=1, ascending=False))
...
def download_all_ludum_data(session_files, path="ludum_data", overwrite=False):
    return utils.map_concurrent(
        download_ludum_data,
        {(row.session_id, row.full_name): (row,) for _, row in session_files.iterrows()},
        max_workers=4,
        overwrite=False,
    )
...
def parse_ludum_gps_data(data):
    ...
    data.pop("line")
    positions = pd.DataFrame.from_dict(data.pop("position"), orient="index").reset_index(drop=True)
    ...
    if "lat" in positions:
        ...
        positions = positions.dropna(subset=["time", "latitude", "longitude", "distance"]).reset_index(drop=True)
...
def read_ludum_path(path, allowed=None, max_workers=10, **kwargs):
    ...
    if allowed is not None:
        activity_paths = {index: (p,) for index, p in id_paths if index in allowed}
    else:
    ...
    activity_info = pd.concat(
        {index: info for index, (info, _) in activity_data.items() if not info.empty}, names=["data_session_id", "user_id"]
    ).droplevel(-1)
