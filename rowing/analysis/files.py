...
def parse_gpx_data(gpx_data):
    ...
    if positions.empty:
        return pd.DataFrame(
            [], columns=["latitude", "longitude", "time", "timeElapsed", "distanceDelta", "distance", "bearing_r", "bearing"]
        )
...
def make_gpx_track(data):
    ...
    for _, point in data.iterrows():
        gpx_segment.points.append(gpxpy.gpx.GPXTrackPoint(latitude=point.latitude, longitude=point.longitude, time=point.time))
...
def process_latlontime(positions, min_speed=0.5):
    ...
    positions.loc[last, "distanceDelta"] = 0.0
    ...
    positions["metrePerSecond"] = np.gradient(positions.distance * 1000, positions.timeElapsed.dt.total_seconds())

    if "velocity_smooth" in positions:
        positions["split"] = pd.to_timedelta(
            500 / positions["velocity_smooth"].clip(min_speed, None).replace({min_speed: np.nan}), unit="s", errors="coerce"
        )
    else:
        positions["split"] = pd.to_timedelta(
            500 / positions["metrePerSecond"].clip(min_speed, None).replace({min_speed: np.nan}), unit="s", errors="coerce"
        )
...
def fit_to_json(fit_file):
    fit_data = fitparse.FitFile(fit_file)
    return [{f.name: f.raw_value for f in record.fields} for record in fit_data.get_messages("record")]
...
def parse_fit_data(fit_file):
    fit_data = fitparse.FitFile(fit_file)
    positions = pd.DataFrame.from_records(
        {f.name: f.value for f in record.fields} for record in fit_data.get_messages("record")
...
def _parse_fit_positions(positions):
    if "position_lat" in positions.columns:
        positions = positions.dropna(subset=["position_lat", "position_long"]).reset_index(drop=True)
...
def activity_data_to_excel(
    ...
    activity_info, best_times, location_timings = splits.process_activities(activities, locations=locations, cols=cols)
    if additional_info is not None:
        activity_info.join(additional_info)
    if sheet_names is None:
        sheet_names = pd.Series(
            activity_info.startTime.dt.strftime("%Y-%m-%d").str.cat(activity_info.index.astype(str), sep=" ")
        )
...
def get_parser():
    parser = argparse.ArgumentParser(description="Analyse gpx data files")

    parser.add_argument(
        "gpx_file", type=str, nargs="*", help="gpx files to process, accepts globs, e.g. activity_*.gpx, default='*.gpx'"
    )
    parser.add_argument(
        "-o",
        "--out-file",
        ...
        help="path to excel spreadsheet to save results to, default='gpx_data.xlsx'",
    )
...
def run(args=None):
    ...
    gpx_files = {f.name: (f,) for gpx_file in options.gpx_file or ["*.gpx"] for f in path.glob(gpx_file)}
    ...
    activities = pd.DataFrame.from_dict({k: data.iloc[-1] for k, data in activity_data.items()}, orient="index").rename(
        columns={"distance": "totalDistance", "time": "endTime"}
    )
    activities.index.name = "filename"
    for k, data in activity_data.items():
        activities.loc[k, "startTime"] = data.time.iloc[0]

    activities.sort_values(by="startTime", ascending=False, inplace=True)
