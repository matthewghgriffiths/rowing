...
def connect_client():
    if "code" in st.query_params:
        ...
        try:
            ...
            st.markdown(
                _strava_html_logo(authorize_url, target="_blank" if "streamlit" in url else "_self"),
...
def get_strava_activities(code, end=None, start=None, limit=None):
    client = get_client(code)
    activities = get_activities(client, end=end, start=start, limit=limit)
    return activities
...
def get_activities(client, end=None, start=None, limit=None):
    if limit:
        ...
        start, end = sorted(pd.to_datetime([start, end]).to_pydatetime())
        limit = None

    activities = pd.json_normalize([a.dict(exclude=["map"]) for a in client.get_activities(end, start, limit)])

    athlete = client.get_athlete()
    activities["athlete"] = name = f"{athlete.firstname} {athlete.lastname}"
    activities["initials"] = utils.initials(name)

    for c in ["elapsed_time", "moving_time"]:
        if c in activities:
            activities[c] = pd.to_timedelta(activities[c], unit="s")
    if "average_speed" in activities:
        activities["average_split"] = pd.to_timedelta(500 / activities["average_speed"], unit="s")

    if "start_date_local" in activities:
        activities.start_date_local = pd.to_datetime(activities.start_date_local)
        activities["date"] = activities.start_date_local.dt.date
        activities["startTime"] = activities.start_date_local.dt.time
...
def load_activity(client, activity_id):
    ...
    data = {k: list(v) for k, v in streams.items()}
    activity_data = pd.DataFrame({k: v[-1][1] for k, v in data.items()}).rename(columns={"heartrate": "heart_rate"})
    activity_data["latitude"] = activity_data.latlng.str[0]
    activity_data["longitude"] = activity_data.latlng.str[1]
    activity_data["time"] = pd.to_timedelta(activity_data.time, unit="s") + pd.Timestamp(activity.start_date)
    return files.process_latlontime(activity_data).drop(columns=["latlng"])


def strava_app(client):
    athlete = client.get_athlete()
    ...
    cols = st.columns(3)
    with cols[0]:
        limit = st.number_input("How many activities to load, set to 0 if selecting date range", value=1, min_value=0, step=1)
    with cols[1]:
        date1 = st.date_input("Select Date", value=pd.Timestamp.today() + pd.Timedelta("1d"), format="YYYY-MM-DD")
    with cols[2]:
        date2 = st.date_input("Range", value=pd.Timestamp.today() - pd.Timedelta("7d"), format="YYYY-MM-DD")
    ...
    activities["elapsed_time"] = (activities["elapsed_time"] + pd.Timestamp(0)).dt.time
    activities["moving_time"] = (activities["moving_time"] + pd.Timestamp(0)).dt.time
    activities["average_split"] = (activities["average_split"] + pd.Timestamp(0)).dt.time
    activities.sort_values("start_date_local", ascending=False, inplace=True)
    sel_activities = inputs.filter_dataframe(
        ...
        column_config={
            "elapsed_time": st.column_config.TimeColumn(format="h:mm:ss", disabled=True),
            "moving_time": st.column_config.DatetimeColumn(format="h:mm:ss", disabled=True),
            "average_split": st.column_config.DatetimeColumn(format="m:ss.S", disabled=True),
        },
    ...
    with st.spinner("Downloading Activities"):
        strava_data = {
            activity.activity: load_strava_activity(client.code, activity.id) for _, activity in sel_activities.iterrows()
        }
    ...
    if st.toggle("Download gpx data"):
        for activity, activity_data in strava_data.items():
            st.download_button(
                ...
                io.BytesIO(files.make_gpx_track(activity_data).to_xml().encode()),
                # type='primary',
