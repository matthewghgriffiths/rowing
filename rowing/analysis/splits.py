...
from . import geodesy
from .utils import distance_to_km, is_pareto_efficient
...
def load_place_locations(loc=None):
    ...
    return {p: pd.read_table(_LOCATION_DATA[p], index_col=0) for p in loc}
...
def find_all_crossing_data(positions, locations=None, thresh=0.5, cols=None):
    ...
    if cols:
        crossings = pd.concat([crossing_times, pd.Series([pd.NaT], [("",) * crossing_times.index.nlevels])]).index
...
def calc_timings(loc_times):
    ...
    loc_timings = pd.DataFrame(times[:, None] - times[None, :], index=loc_times.index, columns=loc_times.index)
...
def find_crossing_times(positions, loc, thresh=0.5):
    ...
    intersections = pd.DataFrame.from_dict(geodesy.path_intersections(close_positions, loc)._asdict())
    ...
    crossing_weights = pd.Series(
        [weight(*geodesy.haversine(intersections.loc[i : i + 2], loc)) for i in crossings],
        index=crossings,
        dtype=float,
    )
    crossings1 = np.clip((crossings + 1), 0, positions.index[-1])
    time_deltas = positions.time[crossings1].values - positions.time[crossings].values
    crossing_times = positions.time[crossings] + time_deltas * crossing_weights
    distance_deltas = positions.distance[crossings1].values - positions.distance[crossings].values
    crossing_distances = positions.distance[crossings] + distance_deltas * crossing_weights
...
def get_distance_to_locations(activity_data, locs=None):
    if not isinstance(locs, pd.DataFrame):
        locs = pd.concat({l: pos.mean() for l, pos in load_place_locations(locs).items()}).unstack()

    loc_dists = locs.T.apply(lambda x: geodesy.haversine_km(activity_data, x))
...
def group_positions(positions, locations=None, freq="1h", thresh=10, update=False):
    ...
    position_groups = positions.groupby(["location", pd.Grouper(key="time", freq=freq)]).size().index.to_frame(False)
    grouped_locations = locations.loc[position_groups["location"]].reset_index()
...
def find_best_times(positions, distance, cols=None):
    ...
    pos_distances = positions.distance + np.minimum(positions.distance.diff(), 0).fillna(0)
    ...
    dist_elapsed = np.interp(end_distances, pos_distances, time_elapsed)
...
def find_all_best_times(positions, distances=None, cols=None):
    ...
    if pd.Index(positions.distance).is_monotonic_increasing:
        return pd.concat(
            {name: find_best_times(positions, distance, cols=cols) for name, distance in distances.items()},
...
def process_activities(activities, locations=None, cols=None):
    ...
    best_times = (
        each_activity.apply(lambda x: find_all_best_times(x.droplevel(0), cols=cols))
        .join(activity_info)
        .reset_index()
        .sort_values(["startTime", "length", "split"], key=lambda s: s.map(distance_to_km) if s.name == "length" else s)
        .set_index([activity_id, "startTime", "totalDistance", "length", "distance"])
    )

    location_timings = (
        (actid, get_location_timings(activity.droplevel(0))) for actid, activity in each_activity if not activity.empty
    )
    location_timings = {actid: timings for actid, timings in location_timings if not timings.empty}
...
def calc_pareto_front(positions):
    ...
    time_diffs = positions.timeElapsed[j].dt.total_seconds().values - positions.timeElapsed[i].dt.total_seconds().values
    dist_diffs = positions.distance[j].values - positions.distance[i].values
...
def calc_time_above_hr(activities, key="activity_id", hrs=None, rescale=3600):
    hrs = pd.RangeIndex(60, 200, 1)
    key = key or activities.index.names[0]
    return (
        activities.reset_index()
        .groupby([key, "heart_rate"])
        .timeDelta.sum()
        .dt.total_seconds()
...
def get_piece_times(crossing_times, start_landmark, finish_landmark):
    ...
    if valid_times.any():
        ...
        piece_data.index = pd.MultiIndex.from_frame(
            start_times.loc[piece_data.index].rename("Start Time").reset_index()[["Start Time", name, leg]]
        )
        piece_data = piece_data.sort_index(level=0)
        piece_distances = (piece_data.distance - piece_data.distance[start_landmark].values[:, None])[col_order]
        piece_time = piece_data["Elapsed time"][col_order]
        avg_split = (piece_time * 0.5 / piece_distances).fillna(pd.Timedelta(0))
        interval_split = (piece_time.diff(axis=1) * 0.5 / piece_distances.diff(axis=1)).fillna(pd.Timedelta(0))
...
def get_interval_averages(X, time, timestamps):
    ...
    group_intervals = pd.cut(time, time_intervals).cat.rename_categories(dict(zip(time_intervals, timestamps.index[1:])))
...
def get_pieces_interval_averages(piece_timestamps, piece_data, time="Time"):
    ...
    for piece, timestamps in piece_timestamps.iterrows():
        ...
        avgP, intervalP = get_interval_averages(data.drop(columns=[time]), data[time], timestamps)
        print(name)
        print(avgP)
    ...
    for k, data in avg_telem.items():
        interval_data[f"Average {k}"] = pd.concat(data, names=["name", "leg"], axis=1).T
    for k, data in interval_telem.items():
        interval_data[f"Interval {k}"] = pd.concat(data, names=["name", "leg"], axis=1).T
...
def get_piece_gps_data(
    ...
    positions = (
        ...
        .loc[
            positions.distance.searchsorted(piece_distances[start_landmark]) - 1 : positions.distance.searchsorted(
    ...
    positions["timeElapsed"] = positions.time - piece_timestamps[start_landmark]
    positions["Distance Travelled"] = np.interp(positions.distance, piece_distances, landmark_distances)
