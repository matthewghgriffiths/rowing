...
def parse_powerline_data(split_data, use_names=True):
    ...
    split_data["Periodic"]["timestamp"] = pd.to_timedelta(split_data["Periodic"].Time, unit="milli") + start_time

    positions = (
        pd.concat(
            {
                "time": pd.to_timedelta(gps_data.Time.squeeze(), unit="milli") + start_time,
    ...
    power["timestamp"] = pd.to_datetime(pd.to_timedelta(power.Time.squeeze(), unit="milli") + start_time)
...
def norm_stroke_profile(stroke_profile, n_res):
    stroke_norm_time = stroke_profile[("Normalized Time", "Boat", "Boat")].squeeze()
...
def compare_piece_telemetry(telemetry_data, piece_data, gps_data, landmark_distances, window=0):
    ...
    for piece, distances in piece_data["Total Distance"].iterrows():
        ...
        if window:
            time_power = power.set_index("timestamp").sort_index()
        ...
        power_epoch = power["timestamp"].astype(int)
        power_adjusted_distance = np.interp(
            power_epoch, gps["timestamp"].astype(int), gps_adjusted_distance, left=np.nan, right=np.nan
        )

        power_distance = power.copy()  # .rename(columns={"Time": "Distance"})
        power_distance["Distance"] = power_adjusted_distance

        power_distance = power_distance[power_distance.Distance.notna()].set_index("Distance")
...
def get_interval_averages(piece_timestamps, telemetry_data):
    ...
    for piece, timestamps in piece_timestamps.iterrows():
        _, name, leg = piece
        power = telemetry_data[name]["power"].sort_values("timestamp").reset_index(drop=True)

        avgP, intervalP = splits.get_interval_averages(
            power.drop("timestamp", axis=1, level=0), power["timestamp"], timestamps
        )
        print(piece)
        print(avgP)
        print(intervalP)
