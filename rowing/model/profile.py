...
def logistics(x, *params, box=1):
    return sum(logistic(x, *args, box=box) for args in zip(*[iter(params)] * 3))
...
def logistics_1(x, *params, box=1):
    return sum(logistic_1(x, *args, box=box) for args in zip(*[iter(params)] * 3))


def guess_peak_times(stroke, stroke_profile):
    t1 = stroke.stroke_time
    return np.r_[0.05, 0.4, (t1 - 0.7) * 0.3 + 0.4, (t1 - 0.7) * 0.6 + 0.4, t1 - 0.3, t1 - 0.1]
...
def fit_stroke_profile(stroke, acc, p0=None, peak_times=None):
    ...
    if p0 is None:
        ...
        peaks = stroke_profile.index[stroke_profile.index.searchsorted(peak_times)]
        p0 = np.c_[stroke_profile.loc[peaks], peaks, np.full_like(peaks, -3)]
