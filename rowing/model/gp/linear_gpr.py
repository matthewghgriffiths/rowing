...
class LinearGPCorrelatedRegression(GaussianProcessRegression):
    ...
    def predict_coef_covar(self, X1):
        ...
        Z1_covar = K11.values - jnp.einsum("ijk,ilm->jlkm", LWk1, LWk1)
        return Z1, Z1_covar
...
def get_linear_gpr(times, X, observations, kernel=None, multi_kernel="cov", **kwargs):
    if kernel == "integral":
        if multi_kernel == "cov":
            kernel = CovMultiKernel(jnp.shape(X)[1], kernel=IntSEKernel())
        elif multi_kernel == "diag":
            kernel = DiagMultiKernel([IntSEKernel() for i in range(jnp.shape(X)[1])])
