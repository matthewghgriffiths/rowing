...
class Banded(_Banded):
    ...
    def ones_like(self, **kwargs) -> Self:
        return self._replace(bands=jnp.ones_like(self.bands, **kwargs)).zero_bands()

    def zeros_like(self, **kwargs) -> Self:
        return self._replace(bands=jnp.zeros_like(self.bands, **kwargs)).zero_bands()

    def full_like(self, fill_value, **kwargs) -> Self:
        return self._replace(bands=jnp.full_like(self.bands, fill_value, **kwargs)).zero_bands()
    ...
    def set(self, dense: jax.Array) -> Self:
        if self.trans:
            dense = dense.T
        return self._replace(bands=_full_to_banded(dense, self.l, self.u, zero=True))
    ...
    def transpose(self, zero: bool = False) -> Self:
        ...
        if banded.trans:
            ...
            return banded._replace(bands=_calc_transpose(*banded), l=-banded.u, u=-banded.l, trans=0)
    ...
    def zero_bands(self) -> Self:
        ...
        return self._replace(bands=jnp.where(mask_bands(l, u, n, n), bands, 0))

    def matmul_banded(self, other: _Banded, lc=None, uc=None) -> Self:
        cls = type(self)
        return cls(*_banded_matmul_banded(*self.transpose(), *cls.from_banded(other).resolve(), lc, uc))
...
class BandedMatrix(Banded):
    ...
    def __new__(cls, bands: Array, l: int, u: int | None = None, trans: int = 0, m: int | None = None):
        if u is None:
            u = l + bands.shape[0] - 1
        if m is None:
            m = bands.shape[1]
...
def _calc_transpose(bands: jax.Array, l: int, u: int, trans: int = 0, m=None) -> jax.Array:
    ...
    return (
        ...
        .flatten()[max(u, 0) : max(u, 0) - nbands]
        .reshape(bands.shape)[::-1]
    )
...
def _banded_matmul_banded(abT, la, ua, ta, ma, bb, lb, ub, tb, mb, lc, uc):
    ...
    cb = jax.vmap(lambda i: jax.vmap(jnp.dot, 1)(bb, jax.lax.dynamic_slice(abT0, (-bb.shape[0] - i, i), bb.shape)))(
        jnp.arange(cbands)
    )
...
def _blas_solve_triangular_banded(args, check_finite=True):
    ...
    shapes = (ab.shape[-2] - 1,)
    return _broadcast_blas(tbsv, shapes, ab, b, lower=lower, trans=trans, overwrite_x=True)
...
def _solve_triangular_banded(ab: BandedMatrix, b, check_finite=False):
    return jax.pure_callback(
        partial(_blas_solve_triangular_banded, check_finite=check_finite),
        jax.ShapeDtypeStruct(b.shape, b.dtype),
        (ab, b),
    )


def fwd_solve_triangular_banded(
    ab: BandedMatrix,
    b,
    check_finite=False,
):
...
def bwd_solve_triangular_banded(res, g):
...
_solve_triangular_banded.defvjp(fwd_solve_triangular_banded, bwd_solve_triangular_banded)


def _blas_banded_triangular_matmul(
    args,
    check_finite=True,
):
...
def banded_triangular_matmul(ab: BandedMatrix, b, check_finite=False) -> jax.Array:
    return _banded_triangular_matmul(BandedMatrix(*ab), b, check_finite=check_finite)


@jax.custom_vjp
def _banded_triangular_matmul(
    ab: BandedMatrix,
    b,
    check_finite=False,
):
    return jax.pure_callback(
        partial(_blas_banded_triangular_matmul, check_finite=check_finite),
        jax.ShapeDtypeStruct(b.shape, b.dtype),
        (*ab, b),
    )


def fwd_banded_triangular_matmul(
    ab,
    b,
    check_finite=False,
):
...
def bwd_banded_triangular_matmul(res, g):
    (ab, b, check_finite) = res
...
_banded_triangular_matmul.defvjp(fwd_banded_triangular_matmul, bwd_banded_triangular_matmul)


def _cholesky_banded(bands, lower=True, check_finite=True, overwrite_ab=False):
    try:
        return linalg.cholesky_banded(bands, overwrite_ab=overwrite_ab, lower=lower, check_finite=check_finite)
...
def cholesky_banded(ab: BandedMatrix):
    ab = as_banded(ab)
    if ab.u and ab.l:
        raise ValueError("Only lower or upper triangular matrices are supported")

    chol = jax.pure_callback(
        partial(_cholesky_banded, lower=bool(ab.l), check_finite=False, overwrite_ab=False),
...
def bwd_cholesky_banded(res, dL):
    Lb, Ab = res
    if Lb.l:
        Lb = Lb.T
    else:
    ...
    w = jnp.ones_like(LTdL.bands, shape=(1 - LTdL.l, 1)).at[0, 0].set(0.5) * 0.5
    phiLTdL = LTdL._replace(bands=LTdL.bands * w).dense().T
    S = solve_triangular_banded(Lb, solve_triangular_banded(Lb, phiLTdL).T)
    dA = Ab.set(S + S.T)
    return (dA,)
...
def bwd_cholesky(L, Lbar):
    ...
    P = Phi(L.T @ Lbar)
    return Phi(
        jsp.linalg.solve_triangular(L, jsp.linalg.solve_triangular(L, (P + P.T), trans=1, lower=True).T, trans=1, lower=True)
    )
...
def bwd_cholesky_scan(l, l1, u1, mR, mDC, mD, lower, Dbar, kbands):
    ...
    Cbar = jsp.linalg.solve_triangular(D, Cbar.T, lower=True, trans=1).T
...
def _bwd_cholesky_lower_banded(Lb, dLb):
    ...
    _, Aband0 = bwd_cholesky_scan(
        l, l1, u1, NB, remainder - l, remainder, False, Dbar, (Lb[:, :remainder], dLb[:, :remainder])
    )
...
def _cholesky_jvp(L, sigma_dot):
    ...
    tmp1 = jax.lax.linalg.triangular_solve(L, sigma_dot, left_side=False, transpose_a=True, conjugate_a=True, lower=True)
    tmp2 = jax.lax.linalg.triangular_solve(L, tmp1, left_side=True, transpose_a=False, lower=True)
...
@jax.custom_jvp
def safe_cholesky(A, min_eig=0.0):
...
def _safe_cholesky_banded_scan(l, mL, mLR, R10, Abandk, min_eig=0.0):
    LR = _banded_to_full(Abandk, l, 0, m=mLR)
    A0_ = LR[:mL]
    A1 = LR[mL:]
    A0 = A0_ + jnp.tril(A0_, -1).T
    L00 = safe_cholesky(A0 - R10 @ R10.T, min_eig=min_eig)
    R10 = jax.lax.linalg.triangular_solve(L00, A1, left_side=False, lower=True, transpose_a=True)
    Lbandk = _full_to_banded(jnp.vstack([L00, R10]), l, 0)
    return R10, Lbandk.T


def _safe_cholesky_banded(Abands, min_eig=0.0):
    ...
    R10, Lbandk = jax.lax.scan(
        partial(_safe_cholesky_banded_scan, l, NB, 2 * NB, min_eig=min_eig), jnp.zeros((NB, NB)), Abandks
    )
    _, Lband1 = _safe_cholesky_banded_scan(l, remainder, 2 * NB, R10[:remainder], Abands[:, -remainder:], min_eig=min_eig)

    Lbands = jnp.c_[Lbandk.reshape(-1, nbands).T, Lband1.T]
    return Lbands


def safe_cholesky_banded(ab: Banded, min_eig=0.0):
