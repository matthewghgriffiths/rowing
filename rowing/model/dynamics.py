...
def follow_bearing(lat, lon, bearing, distance, radius=1):
    d = distance / radius
    lat1 = jnp.arcsin(jnp.sin(lat) * jnp.cos(d) + jnp.cos(lat) * jnp.sin(d) * jnp.cos(bearing))
    lon1 = lon + jnp.arctan2(jnp.sin(bearing) * jnp.sin(d) * jnp.cos(lat), jnp.cos(d) - jnp.sin(lat) * jnp.sin(lat1))
    return lat1, lon1
...
def exp_rolling_lombscargle(times, acceleration, freqs, n_harms, alpha=1e-1):
    ...
    AAT = jnp.concatenate([jnp.eye(2 * n_harms + 1)[None, ...] for i in range(n_freqs)])
    Ax = jnp.zeros((n_freqs, 2 * n_harms + 1))
    init = (AAT, Ax, times[0], freqs, harms, alpha)
    carry, coefs = jax.lax.scan(_exp_rolling_lombscargle, init, xt)
    return coefs
...
def exp_rolling_lombscargle_cho(times, acceleration, freqs, n_harms, alpha=1e-1):
    ...
    Ax = jnp.zeros((n_freqs, 2 * n_harms + 1))
    init = (AAT, Ax, times[0], freqs, harms, alpha)
    carry, coefs = jax.lax.scan(_exp_rolling_lombscargle_cho, init, xt)
    return coefs
