...
def estimate_distance_time_jac(D, P, coef, t_base=T_BASE):
    t, vjp = jax.vjp(lambda P, coef: estimate_distance_time(D, P, coef, t_base=t_base), P, coef)
    return t, vjp(jnp.ones_like(t))


def predict_distance_split(distance, P, P_std, coef, coef_std, t_base=T_BASE):
    est_time, (jac_P, jac_coef) = estimate_distance_time_jac(distance, P, coef, t_base=t_base)
...
def estimate_split_jac(t, P, coef, t_base=T_BASE):
    split, vjp = jax.vjp(lambda P, coef: estimate_split(t, P, coef, t_base=t_base), P, coef)
...
def create_power_law_data(year, duration, power, t_base=T_BASE, **kwargs):
    kwargs.update({"t": year, "log_t": np.log(duration / t_base), "log_p": np.log(power)})
...
def plot_posterior(t, mean, std, ax=None, alpha=0.3, format_splits=True, **kwargs):
    ...
    for m_i, s_i in zip(m.T, s.T):
        ...
        lines.append(line)
        bands.append(ax.fill_between(t, m_i - s_i, m_i + s_i, facecolor=line.get_color(), alpha=alpha))
...
def plot_athlete_data(data, ax=None, ls=":", marker="+", label="", format_splits=False, **kwargs):
    ...
    for piece, piece_data in data.groupby("piece"):
        (l,) = ax.plot(
            piece_data.year, piece_data.split.dt.total_seconds(), label=label + piece, ls=ls, marker=marker, **kwargs
        )
        lines.append(l)
...
def make_predictions(t, make_model, params, inputs, t_pred=None, t_base=T_BASE, W=None):
    ...
    split, split_std = predict_time_split(t_pred, P, P_std, coef, coef_std, t_base=t_base)
    preds = {
        "P": P,
        "P_std": P_std,
        "coef": coef,
...
def plot_model_posterior(t, make_model, params, inputs, t_base=T_BASE, t_plot=None, axes=None, figsize=(10, 8), **kwargs):
    import matplotlib.pyplot as plt

    preds = make_predictions(t, make_model, params, inputs, t_base=t_base)
    P, P_std, coef, coef_std = (preds[k] for k in ("P", "P_std", "coef", "coef_std"))
    ...
    ax = axes[0]
    plot_posterior(t, coef, coef_std, format_splits=False, ax=ax, **kwargs)
    ...
    if t_plot:
        ...
        split, split_std = predict_time_split(t_plot, P, P_std, coef, coef_std, t_base=t_base)

        plot_posterior(t, split, split_std, format_splits=False, ax=ax, label=label, **kwargs)
...
def plot_model_preds(
    ...
    **kwargs,
    ...
    _, _, preds = plot_model_posterior(t, make_model, params, inputs, axes=axes, alpha=alpha, t_base=t_base, **kwargs)
    P, P_std, coef, coef_std = (preds[k] for k in ("P", "P_std", "coef", "coef_std"))
