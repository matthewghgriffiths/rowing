...
def load_raw_pico_data(json_path):
    ...
    for elem in capture["log_entries"]:
        data_groups.setdefault(elem["Name"], {})[elem["Timestamp"]] = elem["Fields"]

    capture_data = {k: pd.DataFrame.from_dict(group, orient="index") for k, group in data_groups.items()}
    return capture_data
...
def process_pico_data(capture_data):
    ...
    valid_gps_data = gps_data.loc[(gps_data.Hor_accuracy < 1000) & (gps_data.HDOP > 0) & (gps_data.HDOP < 5)].copy()
