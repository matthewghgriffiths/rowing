...
def zero_crossings(carry, x):
    ...
    next_carry = (jnp.r_[t1, a1, v1, s1, sv1, var_s1], (lam,))
    val = jnp.r_[a1, v1, s1, sv1, var_s1, sgn_chg]

    return next_carry, val


def find_smoothed_crossings(times, acceleration, lam=100.0):
    tx = jnp.c_[times, acceleration]

    init = (jnp.zeros(6), (lam,))
    carry, vals = jax.lax.scan(zero_crossings, init, tx)
    return pd.DataFrame(vals, index=times, columns=["a", "v", "smooth", "smooth_v", "var_smooth_a", "sgn_chg"])


def find_strokes(crossings, decel_thresh=0.5, accel_thresh=0.5, search_back=1.0):
    peak_data = crossings.loc[crossings.sgn_chg == 1].copy()
    peak_data["DeltaV"] = crossings.v.loc[peak_data.index].diff()

    strokes = peak_data.loc[(peak_data.DeltaV < -decel_thresh) & (peak_data.DeltaV.shift(-1) > accel_thresh)].copy()

    strokes["stroke_end"] = strokes.apply(lambda x: crossings.v[x.name - 1 : x.name].idxmin(), axis=1)
